...
// Методы для архивации товаров
export const archiveProduct = async (id) => {
    try {
        const response = await apiClient.post(`/${id}/archive`);
        return response.data;
    } catch (error) {
        if (error.response && error.response.data && error.response.data.message) {
            throw new Error(error.response.data.message);
        }
        console.error(`Error archiving product with ID ${id}:`, error);
        throw handleApiError(error);
    }
};
...
export const unarchiveProduct = async (id) => {
    try {
        const response = await apiClient.post(`/${id}/unarchive`);
        return response.data;
    } catch (error) {
        if (error.response && error.response.data && error.response.data.message) {
            throw new Error(error.response.data.message);
        }
        console.error(`Error unarchiving product with ID ${id}:`, error);
        throw handleApiError(error);
    }
};

// Получить все товары включая архивированные (для админки)
export const getAllProductsIncludingArchived = async () => {
    try {
        const response = await apiClient.get("/all");
        return response.data;
    } catch (error) {
        console.error("Error fetching all products including archived:", error);
        throw handleApiError(error);
    }
};