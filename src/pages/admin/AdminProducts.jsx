...
import { getProducts, createProduct, updateProduct, deleteProduct, archiveProduct, unarchiveProduct, getAllProductsIncludingArchived } from "../../api/productsApi";
...
const AdminProducts = () => {
    ...
    const fetchProducts = async () => {
        // Загружаем все товары включая архивированные для админки
        const data = await getAllProductsIncludingArchived();
        setProducts(data);
    ...
    const handleArchive = async (id, productName) => {
        const confirmed = window.confirm(
            `Вы уверены, что хотите архивировать товар "${productName}"?\n\n` +
            `Товар будет скрыт из каталога для пользователей, но останется в базе данных и в истории заказов. ` +
            `Его можно будет разархивировать в любое время.`
        );
        ...
        if (confirmed) {
            try {
                ...
                await fetchProducts();
                alert("Товар успешно архивирован!");
            } catch (error) {
                console.error("Ошибка при архивации товара:", error);
                alert("Ошибка при архивации товара: " + error.message);
            }
        }
    };
    ...
    const handleUnarchive = async (id, productName) => {
        const confirmed = window.confirm(
            `Вы уверены, что хотите разархивировать товар "${productName}"?\n\n` +
            `Товар снова появится в каталоге для пользователей.`
        );
        ...
        if (confirmed) {
            try {
                ...
                await fetchProducts();
                alert("Товар успешно разархивирован!");
            } catch (error) {
                console.error("Ошибка при разархивации товара:", error);
                alert("Ошибка при разархивации товара: " + error.message);
            }
        }
    };
                                ...
                                <td>
                                    <span 
                                        style={{ 
                                            ...
                                            fontSize: '12px',
                                            fontWeight: 'bold',
                                            backgroundColor: product.archived ? '#dc3545' : '#28a745',
                                            color: 'white'
                                        }}
                                    >
                                        {product.archived ? 'Архивирован' : 'Активен'}
                                    </span>
                                </td>
                                <td>
